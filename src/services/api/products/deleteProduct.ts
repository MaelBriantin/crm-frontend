...
export const deleteProduct = async (product: ProductType, callToast: CallToastProps, refreshProducts: { (): Promise<void>; (): void; }, closeModal?: { (): void; (): void; }) => {
    let productsResponse: ProductType[] = [];
    try {
        const response: APIResponseFormat<ProductType> = await fetchAPI(`/api/products/${product.id}`, 'DELETE');
        handleAPIResponse<ProductType>(
            response,
            async (products) => {
                productsResponse = products as ProductType[];
                await refreshProducts();
                callToast('success', `Le produit ${product.name} a bien été supprimé.`, 3000);
