...
export const getColor = (
  columnColor: ColumnColorProps[] | undefined,
  value: string
) => {
  if (columnColor && Array.isArray(columnColor)) {
    const wildcard = columnColor.find(
      (color) => typeof color.value === "string" && color.value.startsWith("*")
    );
    if (wildcard) {
      if (wildcard.value === "*") {
        return {
 ...
 */
export const highlightSearchedValue = (
  searchedValue: string | number | undefined,
  value: string | number | boolean | string[]
): string | number | boolean | string[] => {
  if (typeof value === "string" || typeof value === "number") {
    const isSearched =
      searchedValue &&
      String(value).toLowerCase().includes(String(searchedValue).toLowerCase());
    if (isSearched) {
      return String(value).replace(
        new RegExp(String(searchedValue), "gi"),
