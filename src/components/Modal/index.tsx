...
export const Modal: React.FC<ModalProps> = ({
  ...
  const { isOpen, closeModal, startCloseAnimation, setStartCloseAnimation } =
    useModal();
  ...
  return (
    <Container $closeModalAnimation={startCloseAnimation} $isOpen={isOpen}>
      <ModalStructure
        $closeModalAnimation={startCloseAnimation}
        $style={{ height, width }}
      >
        <ModalTitle>
          <Title>
          ...
          <CloseButton
            ...
            onClick={!disableClose ? closeModal : () => {}}
          >
            <VscChromeClose />
          </CloseButton>
        </ModalTitle>
        <ModalContent $actions={!!actions}>{children}</ModalContent>
...
const Container = styled.div<{
  ...
  ${({ $closeModalAnimation }) =>
    $closeModalAnimation
      ? css`
          animation: ${ContainerCloseAnimation} 250ms;
        `
      : ""};
...
const ModalStructure = styled.div<{
  $closeModalAnimation: boolean;
  $style: { height: string | undefined; width: string | undefined };
}>`
  ...
  ${({ $closeModalAnimation }) =>
    $closeModalAnimation
      ? css`
          animation: ${ModalCloseAnimation} 250ms;
        `
      : ""};
`;
...
const ModalTitle = styled.div`
  border-radius: ${theme.materialDesign.borderRadius.rounded};
  border-radius: ${theme.materialDesign.borderRadius.rounded}
    ${theme.materialDesign.borderRadius.rounded} 0 0;
  // padding: 10px 0;
  width: 100%;
  display: flex;
